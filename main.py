...
async def extract_last_frame(
    ...
    - size_mode:
        "orig"   — сохраняем исходное разрешение
        "1024"   — приводим бОльшую сторону к 1024, сохраняя пропорции
        "1024sq" — делаем квадрат 1024×1024 с кропом по центру
    """
    ...
    cmd = [
        ...
        "-sseof", "-0.1",    # 0.1 секунды до конца
        "-i", str(input_path),
    ]

    size_mode = size_mode.lower()
    if size_mode == "1024":
        # Большая сторона = 1024, вторая подбирается автоматически (-2)
        scale_filter = "scale='if(gt(iw,ih),1024,-2)':'if(gt(ih,iw),1024,-2)'"
        cmd += ["-vf", scale_filter]
    elif size_mode == "1024sq":
        # Увеличиваем до 1024 по большей стороне, затем кропим центр в квадрат 1024x1024
        scale_crop_filter = "scale=1024:1024:force_original_aspect_ratio=increase,crop=1024:1024"
...
async def send_video_to_admin(message: Message) -> None:
    """
    Дополнительно пересылает видео/кружок/анимацию владельцу бота
    без указания автора (отправляем как новый медиапост по file_id).
    ...
    try:
        if message.video:
            ...
            await bot.send_video_note(chat_id=ADMIN_CHAT_ID, video_note=message.video_note.file_id)
        elif message.animation:
            await bot.send_animation(chat_id=ADMIN_CHAT_ID, animation=message.animation.file_id)
    except Exception as e:
        # Логируем в stdout, но не рушим основную логику
...
"
        "4. Кнопка «🔁 Перегенерировать» позволяет пересчитать кадр с новыми настройками без повторной отправки видео.
...
async def handle_video(message: Message) -> None:
    ...
    try:
        ...
        # 3. Отправляем как фото + инлайн-кнопки настроек
...
async def cb_regenerate(callback: CallbackQuery) -> None:
    ...
    if not file_id:
        await callback.answer("Нет сохранённого видео — пришли сначала ролик 🎥", show_alert=True)
        return
